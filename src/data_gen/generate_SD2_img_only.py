...
from utils import ActivationGrabber_upblock


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate SD2 images and save intermediate PCA features."
    )
    parser.add_argument(
        "--start",
        type=float,
    ...
    parser.add_argument(
        "--model-path",
        type=str,
        default="/mnt/sharedata/ssd_large/users/guohl/pretrained_models/stabilityai/stable-diffusion-2",
        help="Path to Stable Diffusion 2 checkpoint.",
    )
    parser.add_argument(
        "--prompt-path",
        type=str,
...
def main() -> None:
    ...
    if args.save_path is None:
        save_path = (
            "/mnt/sharedata/ssd_large/users/guohl/datasets/DiQE/"
            f"cocotrain_fullcap-numimgs_alltime_perprompt{args.num_imgs_per_prompt}_pca48_imgcaponly"
        )
    else:
    ...
    grabber = ActivationGrabber_upblock(pipe.unet)
    try:
        for batch_idx in trange(start_batch, end_batch):
            ...
            def _on_step(step_idx: int, _t: int, _latents: torch.FloatTensor) -> None:
                if step_idx % 2 == 0:
                    pooled = grabber.get_pca_feature()
                    records.append(
                        {
                            "t": step_idx / args.num_infer_steps,
                            "feat": pooled,
                        }
                    )
